...
def push_to_discord(article_data: Dict[str, Any], ai_result: Dict[str, Any]) -> bool:
    """
    将处理后的文章推送到 Discord。
    ...
    impact_analysis = ai_result.get("impact_analysis", "无分析。")

    # 容错：确保 bullet_points 至少有 3 项，避免索引越界
    while len(bullet_points) < 3:
        bullet_points.append("无")

    # 严格遵循需求文档的排版模板
    markdown_content = (
        f"🔥 **[{title}]** (AI 评分: {score}/10)\n\n"
        f"📝 **核心速览**：\n"
        f"• {bullet_points[0]}\n"
        f"• {bullet_points[1]}\n"
        f"• {bullet_points[2]}\n\n"
...
if __name__ == "__main__":
    ...
    success = push_to_discord(mock_article, mock_ai_result)
    
    if success:
        print("\n测试通过！请查看你的 Discord 频道，检查排版效果是否满意。")
    else:
        print("\n测试失败，请检查 .env 中的 Webhook URL 是否正确。")
